...
public:
	...
	bool	AddItem(const BString &string) { return AddItem(string.String()); }
	...
	bool	RemoveItem(const BString &item) { return RemoveItem(item.String()); }
	bool	RemoveItem(BString *item);
	BString *RemoveItem(int32 index);
	bool 	RemoveItem(const char *string);
